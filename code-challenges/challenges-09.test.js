...
const mapCurrentEvents = () => {
 let currentEventArray = currentEvents.news.map( item => {
...
const countNumberOfElements = (arr) => { //this works, but there has to be a better way!
  let elements = 1;
...
const returnNames = (arr) => {
  let characterNames = [];
   arr.reduce ( (character, cur) => {
    return characterNames.push(cur.name);
  },0)
...
const reversedString = (str) => {
  ...
  let strReverse = strArray.reduce ((character, cur) => {
