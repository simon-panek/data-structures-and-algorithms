...
const createServer = () => {
  ...
  app.get('/', home);
  app.delete('/things/1', deleteFunc);

  function home (req, res) {
    
      res.status(200).send('Dumb message that shouldnt be required.');
    
  }

  function deleteFunc (req, res) {

      res.status(405).send('Dumb message that shouldnt be required.');
...
const toTitleCase = (arr) => {
  let titleCase = [];
  arr.forEach( title => {
    let firstLetter = title.substring(0,1);
    let remainingLetters = title.substring(1);
    let firstCap = firstLetter.toUpperCase();
    titleCase.push(firstCap + remainingLetters);
  })
  return titleCase;
};
...
let biggerThanLuke = (arr) => {
  let massiveCharacters = '';

  arr.forEach ( character => {
    if(character.mass > 77) {
      massiveCharacters += ' - ' + character.name;
    }
  })
  massiveCharacters = massiveCharacters.substring(3);

  return massiveCharacters;
};
