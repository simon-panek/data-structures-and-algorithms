...
const oddValues = (arr) => {
  let oddArray = arr.filter ( (array, index) => {
    return (array % 2);
  })
  return oddArray;
};
...
const filterStringsWithVowels = (arr) => {
  ...
  let vowelWords = arr.filter (words=> {
    let regex = /\w*[AEIOUaeiou]\w*/g;
    let matches = regex.test(words);
    return matches;
  })
  return vowelWords;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let uniqueNumsArray = arr.filter( (item, indx) => {
    let contain = false;
    forbiddenValues.forEach (number => {
        if (item === number) {
          contain = true;
        }
    })
