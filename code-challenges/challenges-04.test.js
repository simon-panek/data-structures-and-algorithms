...
const isNum = (input) => {
  ...
  let test = numCheck.test(stringInput);
...
const isCapitalized = (str) => {
  ...
  let firstLetters = /\b[A-Z][a-z]+/g;

  arrayOfCapitals = str.match(firstLetters);

  if(str.match(firstLetters) === null){
    arrayOfCapitals = [];
  } 

  return arrayOfCapitals;
...
const citiesAtoJ = (arr) => {
  ...
  arr.forEach(loopTest => {
    
    let matchCheck = /^[A-J]/g;

    let test = matchCheck.test(loopTest);

    if (test === true){
      
      testArray.push(loopTest);
