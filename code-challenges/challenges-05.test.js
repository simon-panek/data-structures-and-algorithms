...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(star => {
    ...
    content.find('h2').text(star.name);
    content.find('h3').text(star.height);
    content.find('p').text(star.eye_color);
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach( item => {
    ...
    let shortItem = item.slice(1);

    let sliceIndex = (shortItem.indexOf(' ', 2) +1);

    result.push(shortItem.slice(sliceIndex));
  });
  return result;
};
